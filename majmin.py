	...
	def getMajorMinor(self, mval):
		...
		for columnName, columnData in self.pdf.items():
			...
			if len(allelecounts.keys()) == 1:
				...
				print(allelecounts.keys())
				print("ERROR: cannot write Sequoia output.")
				print(str(num) + " alleles detected at " + columnName + ".")
				print("Sequoia option only works with biallelic data.")
