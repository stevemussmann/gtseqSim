...
class Colony():
	...
	def __init__(self, df, pop1Name, pop2Name=None):
		self.df = df
		...
		# drop parents from pandas dataframe
		self.df = self.df[~self.df.index.str.contains(pop1Name)]
		if pop2Name:
			self.df = self.df[~self.df.index.str.contains(pop2Name)]
